...
from queue_api.queue_api.db.atlas_database import atlas_session
...
class QueueJoin(Resource):
    def post(self):
        ...
        try:
            ...
            query = text("""SELECT COUNT(1) FROM account WHERE name=:name AND password=:password""")
            account = game_server_session.query(query, **payload)
